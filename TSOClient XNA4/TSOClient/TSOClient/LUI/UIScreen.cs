    ...
    public class UIScreen : GameScreen
    {
        ...
        public void LoadBackground(uint id_0, uint id_1, string TextureName)
        {
            ...
            if (ID != 0x00)
            {
                ...
                m_Backgrounds.Add(TextureConverter.BitmapToTexture(ScreenMgr.GraphicsDevice, new System.Drawing.Bitmap(TextureStream)));
            }
            else
            {
        ...
        public virtual UIButton CreateButton(uint id_0, uint id_1, int X, int Y, int Alpha, bool Disabled, string StrID)
        {
            ...
            try
            {
                ...
                Texture = TextureConverter.BitmapToTexture(ScreenMgr.GraphicsDevice, new System.Drawing.Bitmap(TextureStream));
            }
            catch (FAR3Exception)
            {
                TextureStream = new MemoryStream(ContentManager.GetResourceFromLongID(ID));
                Texture = Texture2D.FromStream(m_ScreenMgr.GraphicsDevice, TextureStream);
                TextureStream.Close();
            }
        ...
        public void CreateImage(uint id_0, uint id_1, int X, int Y, int Alpha, string StrID)
        {
            ulong ID = (ulong)(((ulong)id_0) << 32 | ((ulong)(id_1 >> 32)));
            MemoryStream TextureStream = new MemoryStream(ContentManager.GetResourceFromLongID(ID));
            Texture2D Texture = TextureConverter.BitmapToTexture(ScreenMgr.GraphicsDevice, new System.Drawing.Bitmap(TextureStream));
        ...
        public void CreateLoginDialog(int X, int Y)
        {
            ...
            Texture2D DiagTexture = TextureConverter.BitmapToTexture(ScreenMgr.GraphicsDevice, new System.Drawing.Bitmap(TexStream));
