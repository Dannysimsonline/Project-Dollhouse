...
namespace TSOClient.LUI
{
    ...
    public class ImgInfoPopup
    {
        ...
        public ImgInfoPopup(int X, int Y, int ID, string ImgFile, int TextID, UIScreen Screen)
        {
            ...
            m_HintImg = Texture2D.FromStream(Screen.ScreenMgr.GraphicsDevice, File.Open(GlobalSettings.Default.StartupPath + 
                "uigraphics\\hints\\" + ImgFile, FileMode.Open));

            //dialog_backgroundtemplate.tga
            MemoryStream TexStream = new MemoryStream(ContentManager.GetResourceFromLongID(0xe500000002));
            m_DiagImg = TextureConverter.BitmapToTexture(Screen.ScreenMgr.GraphicsDevice, new System.Drawing.Bitmap(TexStream));
